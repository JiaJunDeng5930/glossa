...
const fields: Record<string, FieldFeedback> = {
  ...
  learningWindowDays: numberFeedback("learningWindowDays", "学习窗口（天）", SETTINGS_RULES.learningWindowDays),
  glossCacheTtlMs: numberFeedback("glossCacheTtlHours", "缓存有效期（小时）", SETTINGS_RULES.glossCacheTtlMs, 3_600_000),
  ...
  "ai.requestTimeoutMs": numberFeedback("aiRequestTimeoutSeconds", "AI 超时（秒）", SETTINGS_RULES.ai.requestTimeoutMs, 1_000),
  "anki.requestTimeoutMs": numberFeedback("ankiRequestTimeoutSeconds", "Anki 超时（秒）", SETTINGS_RULES.anki.requestTimeoutMs, 1_000),
  "jev.requestTimeoutMs": numberFeedback("jevRequestTimeoutSeconds", "Jev 超时（秒）", SETTINGS_RULES.jev.requestTimeoutMs, 1_000),
  "anki.duplicatePromptMs": numberFeedback("duplicatePromptSeconds", "重复制卡提示（秒）", SETTINGS_RULES.anki.duplicatePromptMs, 1_000)
};
