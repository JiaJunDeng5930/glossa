...
Each token wrapper contains the gloss label above the original word and centers both children on the same horizontal axis. The wrapper reserves the wider of the label and source word, so dense labels create uneven word spacing through normal browser layout instead of covering neighboring labels or page text.
