...
The content script renders glosses in the page text flow after translation is active for the page. Activation starts from `settings.autoTranslateEnabled`, the action popup message `glossa.activateTranslation`, or `settings.translateShortcutKey`. `createGlossOverlay` keeps the `#glossa-overlay` host for extension identity and settings variables, then wraps each rendered source word with an inline `data-glossa-token` span.
