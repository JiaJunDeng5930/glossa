...
Content translation has a frame-local activation flag. It starts from `settings.autoTranslateEnabled`, accepts an explicit state when the action popup broadcasts `glossa.setTranslationState` across the tab, toggles when the frame receives `settings.translateShortcutKey`, and resets to the setting value after a route URL change. The popup reads frame 0 as the tab state before broadcasting; a state read that activates a new route also starts its viewport scan. Toggling off clears queued scans, closes gloss ports, unwraps rendered tokens, and keeps mutation and scroll rescans idle while the page is inactive. Live generation-setting changes close current gloss sessions, clear rendered wrappers, and rescan; refresh keys include open shadow roots and remain pending until each prior label returns to the viewport. Concurrent known-list loads use a revision guard so only the latest selection is applied.
...
Known-word filter assets live in `assets/known-wordlists/`. `junior-high` is the unstarred compulsory-education subset and `senior-high` is the full 3000-word appendix from the Ministry of Education high-school English curriculum standard zip. Extra filter presets cover CET-4, CET-6, TOEFL, GRE, and COCA 20000. The options page opens known lexicon records in a dialog grouped by first letter and lets the user add, remove, or clear those records manually. Resetting card history clears `cardCache`, `cardedWords`, and legacy `VocabularyRecord.ankiNoteIds` while leaving Anki notes unchanged.

Plugin UI follows `DESIGN.md`: warm editorial paper, ink typography, fine rules, vermillion actions, restrained motion, and square document-like surfaces. Connection test buttons stay directly below Reasoning effort and Anki deck, and their local outputs carry explicit success or error state.
