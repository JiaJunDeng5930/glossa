...
async function assertMobileNavigation(page: Page, viewport: { width: number; height: number }): Promise<void> {
...
async function assertStoryCopySeparation(page: Page): Promise<void> {
  const separation = await page.evaluate(() => {
    ...
    if (!chapter) throw new Error("No visible story chapter.");
    return {
...
test("landing page keeps its story, CTA, and responsive layout intact", async ({ page }) => {
    ...
    assert.equal(await page.locator("#hero-title").textContent(), "在英文网页上显示生词的中文释义。");
    assert.equal(await page.locator(".hero-caption span").count(), 0);
    assert.match(await page.locator(".margin-note").textContent() ?? "", /根据所在句子，\s*显示对应的词义。/);
    ...
    const geometry = await story.evaluate((node) => ({
      ...
      distance: (node as HTMLElement).offsetHeight - innerHeight,
    }));
