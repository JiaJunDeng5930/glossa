...
describe("frontend user error messages", () => {
  ...
  it("maps Anki diagnostics to UI copy", () => {
    ...
    expect(userMessageForError({ reason: "service-error", message: "model was not found: Basic", code: "anki-model-not-found", service: "anki" }, "anki")).toBe("Anki 卡片模板不存在");
    expect(userMessageForError({ reason: "service-error", message: "Anki deck was not found", code: "anki-deck-not-found", service: "anki" }, "anki")).toBe("Anki 牌组不存在");
